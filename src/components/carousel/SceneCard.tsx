...
const SceneCard: React.FC<SceneCardProps> = ({ scene, className = '', onClick, imageDimensions = 'square' }) => {
  ...
  if (isLoading) {
    return (
      <div className={`bg-dark-700 rounded-lg p-4 animate-pulse ${className}`}>
        <div className={`${aspectRatioClass} bg-dark-600 rounded mb-3`}></div>
  ...
  if (hasError) {
    return (
      <div className={`bg-dark-700 rounded-lg p-4 ${className}`}>
        <div className={`${aspectRatioClass} bg-dark-600 rounded mb-3 flex items-center justify-center`}>
  ...
  return (
    <div 
      ...
      onClick={onClick}
    >
      <div className={`${aspectRatioClass} bg-dark-600 rounded mb-3 overflow-hidden`}>
        <img 
          src={scene.imageUrl} 
